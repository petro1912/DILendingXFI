...
library TransferLib {
    ...
    function transferCollateral(State storage state, address _from, address _to, uint256 _amount) external {
        _validateTransfer(_from, _to, _amount);
        state.tokenConfig.collateralToken.safeTransferFrom(_from, _to, _amount);
    }

    function transferCollateral(State storage state, address _to, uint256 _amount) external {
        _validateTransfer(_to, _amount);
        state.tokenConfig.collateralToken.safeTransferFrom(address(this), _to, _amount);
    }
